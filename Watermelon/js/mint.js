...
Mint.save = function() 
{
	...
	var path	= 'http://www.culinaryschools.org/mint/?record&key=363548524e6b4d737768363739363130757a6e47373167';
...
{
	return /^([^:]+):\/\/([a-z0-9]+[\._-])*(culinaryschools\.org)/i.test(url);
};
function Mint_SI_IO_save()
{
	var now		= new Date();
	var then	= now.getTime() + 300;
	var path	= 'http://www.culinaryschools.org/mint/pepper/shauninman/outbound/click.php?'+now.getTime();
	path 		= path.replace(/^https?:/, window.location.protocol);
